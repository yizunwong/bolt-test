...
export default function PlansPage() {
  ...
  const plans = [
    {
      ...
      popular: false,
      description: 'Essential health coverage for individuals with basic medical needs',
      features: [
        'Emergency care',
        'Primary care visits',
    ...
    },
    {
      ...
      popular: true,
      description: 'Comprehensive health coverage with enhanced benefits and global network',
      features: [
        'All Basic features',
    ...
    },
    {
      ...
      popular: false,
      description: 'Premium healthcare with global coverage and exclusive benefits',
      features: [
      ...
      limitations: [
        'High deductible for certain elective procedures'
      ],
      bestFor: 'High-net-worth individuals and executives requiring premium care',
      rating: 4.9,
      reviews: 312
    },
    {
      ...
      popular: false,
      description: 'Essential travel protection for domestic and international trips',
      features: [
    ...
    },
    {
      ...
      popular: true,
      description: 'Comprehensive travel insurance with enhanced benefits for frequent travelers',
      features: [
        'All Basic features',
    ...
    },
    {
      ...
      popular: false,
      description: 'Long-term travel insurance designed for digital nomads and remote workers',
      features: [
        'All Premium features',
    ...
    },
    {
      ...
      popular: false,
      description: 'Essential crop protection against major weather events and natural disasters',
      features: [
    ...
    },
    {
      ...
      popular: true,
      description: 'Comprehensive agricultural insurance with advanced monitoring and protection',
      features: [
        'All Basic features',
  ...
  const sortedPlans = [...filteredPlans].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'coverage':
        return parseInt(b.coverage.replace(/\D/g, '')) - parseInt(a.coverage.replace(/\D/g, ''));
      case 'rating':
  ...
  const selectedPlansData = plans.filter(plan => selectedPlans.includes(plan.id));
      ...
      <section className="relative overflow-hidden pt-24 pb-20 px-4">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-xl md:text-2xl text-slate-600 dark:text-slate-300 mb-12 max-w-4xl mx-auto">
            Explore our comprehensive range of blockchain-powered insurance plans designed to fit your specific needs and budget.
          </p>
        </div>
      </section>
              ...
              >
                <div className={`w-12 h-12 rounded-xl ${
                  ...
                  <category.icon className={`w-6 h-6 ${
                    selectedCategory === category.id ? 'text-white' : 'text-slate-600 dark:text-slate-300'
                  }`} />
                </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-center mb-8">
            <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-4 md:mb-0">
              {selectedCategory === 'all' ? 'All Insurance Plans' : `${categories.find(c => c.id === selectedCategory)?.name} Plans`}
            </h2>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                ...
                {showComparison ? null : <span className="ml-2 text-xs">({selectedPlans.length}/3)</span>}
              </Button>
            </div>
          </div>
          ...
          {showComparison && selectedPlans.length > 0 && (
            <Card className="glass-card rounded-2xl mb-8">
              <CardContent className="p-6">
                <h3 className="text-xl font-semibold text-slate-800 dark:text-slate-100 mb-4">Plan Comparison</h3>
                
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr>
                        <th className="text-left p-3 text-slate-700 dark:text-slate-300 font-medium">Feature</th>
                        {selectedPlansData.map(plan => (
                          <th key={plan.id} className="text-center p-3 text-slate-700 dark:text-slate-300 font-medium">
                            <div className="flex flex-col items-center">
                              <div className={`w-10 h-10 rounded-lg bg-gradient-to-r ${plan.gradient} flex items-center justify-center mb-2`}>
                    ...
                    <tbody>
                      <tr className="border-t border-slate-200 dark:border-slate-700">
                        <td className="p-3 font-medium text-slate-800 dark:text-slate-100">Coverage</td>
                        {selectedPlansData.map(plan => (
                          <td key={plan.id} className="p-3 text-center font-semibold text-emerald-600 dark:text-emerald-400">
                            {plan.coverage}
                          </td>
                        ))}
                      </tr>
                      <tr className="border-t border-slate-200 dark:border-slate-700">
                        <td className="p-3 font-medium text-slate-800 dark:text-slate-100">Premium</td>
                        {selectedPlansData.map(plan => (
                          <td key={plan.id} className="p-3 text-center text-slate-700 dark:text-slate-300">
                            {plan.premium}
                          </td>
                        ))}
                      </tr>
                      <tr className="border-t border-slate-200 dark:border-slate-700">
                        <td className="p-3 font-medium text-slate-800 dark:text-slate-100">Rating</td>
                        {selectedPlansData.map(plan => (
                          <td key={plan.id} className="p-3 text-center text-slate-700 dark:text-slate-300">
                            <div className="flex items-center justify-center">
                              <Star className="w-4 h-4 text-yellow-500 fill-current mr-1" />
                              <span>{plan.rating}</span>
                              <span className="text-xs text-slate-500 ml-1">({plan.reviews})</span>
                            </div>
                          </td>
                        ))}
                      </tr>
                      <tr className="border-t border-slate-200 dark:border-slate-700">
                        <td className="p-3 font-medium text-slate-800 dark:text-slate-100">Features</td>
                        {selectedPlansData.map(plan => (
                          <td key={plan.id} className="p-3 text-center text-slate-700 dark:text-slate-300">
                            <div className="text-sm">{plan.features.length} features</div>
                          </td>
                        ))}
                      </tr>
                      <tr className="border-t border-slate-200 dark:border-slate-700">
                        <td className="p-3 font-medium text-slate-800 dark:text-slate-100">Best For</td>
                        {selectedPlansData.map(plan => (
                          <td key={plan.id} className="p-3 text-center text-sm text-slate-600 dark:text-slate-400">
                            {plan.bestFor}
                          </td>
                        ))}
                      </tr>
                      <tr className="border-t border-slate-200 dark:border-slate-700">
                        <td className="p-3 font-medium text-slate-800 dark:text-slate-100">Action</td>
                        {selectedPlansData.map(plan => (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {sortedPlans.map((plan) => (
              <Card key={plan.id} className={`glass-card rounded-2xl card-hover relative ${plan.popular ? 'ring-2 ring-emerald-500' : ''}`}>
                {plan.popular && (
                ...
                <CardContent className="p-8">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h3 className="text-xl font-semibold text-slate-800 dark:text-slate-100">{plan.name}</h3>
                        <div className="flex items-center text-sm text-slate-600 dark:text-slate-400">
                          <Star className="w-4 h-4 text-yellow-500 fill-current mr-1" />
                          <span>{plan.rating}</span>
                          <span className="text-xs ml-1">({plan.reviews} reviews)</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {showComparison && (
                      ...
                      >
                        {selectedPlans.includes(plan.id) && <CheckCircle className="w-4 h-4" />}
                      </button>
                    )}
                  </div>
                  ...
                  <div className="mb-6">
                    <h4 className="font-medium text-slate-800 dark:text-slate-100 mb-3">Key Features:</h4>
                  ...
                  <div className="mb-6">
                    <h4 className="font-medium text-slate-800 dark:text-slate-100 mb-3">Limitations:</h4>
                    <div className="space-y-2">
                      {plan.limitations.slice(0, 2).map((limitation, limitIndex) => (
                        <div key={limitIndex} className="flex items-center text-sm text-slate-600 dark:text-slate-400">
            ...
            {specialOffers.map((offer, index) => (
              <Card key={index} className="glass-card rounded-2xl card-hover">
                <CardContent className="p-8">
                  ...
                  <h3 className="text-xl font-semibold text-slate-800 dark:text-slate-100 mb-3 text-center">{offer.title}</h3>
                  <p className="text-slate-600 dark:text-slate-400 mb-6 text-center">{offer.description}</p>
                  
                  <div className="bg-slate-100 dark:bg-slate-700 p-3 rounded-lg text-center mb-4">
                    <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">Promo Code</div>
                    <div className="font-mono font-bold text-emerald-600 dark:text-emerald-400">{offer.code}</div>
                  </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {billingTerms.map((term, index) => (
              <Card key={index} className="glass-card rounded-2xl">
                <CardContent className="p-6">
                  <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">{term.title}</h3>
          ...
          <div className="mt-12 p-6 bg-blue-50/50 dark:bg-blue-900/20 rounded-2xl border border-blue-200 dark:border-blue-800">
            <div className="flex items-start space-x-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-blue-800 dark:text-blue-200 mb-2">Important Information</h3>
                <p className="text-blue-700 dark:text-blue-300 mb-4">
                  All plans are subject to our standard terms and conditions. Prices may vary based on individual risk assessment and coverage options. Cryptocurrency prices are locked at the time of purchase to protect against volatility.
                </p>
                <p className="text-blue-700 dark:text-blue-300">
                  For detailed policy information, please review the full policy documentation provided after purchase. All smart contracts are audited by leading security firms and fully transparent on the blockchain.
                </p>
              </div>
            </div>
          </div>
