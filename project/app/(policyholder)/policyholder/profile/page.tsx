        ...
        <div className="page-header">
          <div className="page-header-content">
            ...
            <div>
              ...
              <p className="page-header-subtitle">Manage your account information and preferences</p>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-1">
            <Card className="glass-card rounded-2xl">
              <CardContent className="p-6 text-center">
                ...
                <p className="text-slate-600 dark:text-slate-400 mb-4">{profileData.occupation}</p>
          ...
          <div className="lg:col-span-3">
            <Tabs defaultValue="personal" className="space-y-6">
              <TabsList className="grid w-full grid-cols-4 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
                ...
                <TabsTrigger value="kyc" className="rounded-lg">KYC Status</TabsTrigger>
              ...
              <TabsContent value="kyc">
                <Card className="glass-card rounded-2xl">
                  <CardHeader>
                    <CardTitle className="text-xl text-slate-800 dark:text-slate-100">KYC Verification Status</CardTitle>
                    <p className="text-slate-600 dark:text-slate-400">Your identity verification status for compliance</p>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    {Object.entries(kycStatus).map(([key, value]) => (
                      <div key={key} className="flex items-center justify-between p-4 bg-slate-50/50 dark:bg-slate-700/30 rounded-xl">
                        <div className="flex items-center space-x-4">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                            value.status === 'verified' ? 'bg-gradient-to-r from-emerald-500 to-green-600' :
                            value.status === 'pending' ? 'bg-gradient-to-r from-yellow-500 to-orange-500' :
                          ...
                          <div>
                            ...
                            {value.date && (
                              <p className="text-sm text-slate-600 dark:text-slate-400">
                                Verified on {new Date(value.date).toLocaleDateString()}
                              </p>
                            )}
                          </div>
                        </div>
                        <Badge className={`status-badge ${getStatusColor(value.status)}`}>
                          {getStatusIcon(value.status)}
                          <span className="ml-1 capitalize">{value.status}</span>
                        </Badge>
                      </div>
                    ))}

                    {Object.values(kycStatus).some(v => v.status === 'pending') && (
                      <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700/50 rounded-xl">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <h4 className="font-semibold text-yellow-800 dark:text-yellow-200">Pending Verification</h4>
                        </div>
                        <p className="text-yellow-700 dark:text-yellow-300 text-sm">
                          Some documents are still under review. This may take 1-3 business days to complete.
                        </p>
                      </div>
                    )}
                  </CardContent>
              ...
              <TabsContent value="notifications">
                <Card className="glass-card rounded-2xl">
                  <CardHeader>
                    ...
                    <p className="text-slate-600 dark:text-slate-400">Choose how you want to receive updates</p>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    <div>
                      ...
                      <div className="space-y-3">
                        {[
                          { key: 'emailClaims', label: 'Claim updates and status changes' },
                          { key: 'emailPremiums', label: 'Premium payment reminders' },
                          { key: 'emailOffers', label: 'New policy offers and promotions' }
                        ].map(({ key, label }) => (
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {[
                          { key: 'pushClaims', label: 'Instant claim notifications' },
                          { key: 'pushPremiums', label: 'Payment due alerts' },
                          { key: 'pushOffers', label: 'Special offers and deals' }
                        ].map(({ key, label }) => (
                          <div key={key} className="flex items-center justify-between p-3 bg-slate-50/50 dark:bg-slate-700/30 rounded-lg">
                            <span className="text-slate-700 dark:text-slate-300">{label}</span>
                            <Button
                    ...
                    <div>
                      <h4 className="font-semibold text-slate-800 dark:text-slate-100 mb-4">SMS Notifications</h4>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between p-3 bg-slate-50/50 dark:bg-slate-700/30 rounded-lg">
                          <span className="text-slate-700 dark:text-slate-300">Urgent security alerts only</span>
                          <Button
                            variant={notifications.smsUrgent ? "default" : "outline"}
                            size="sm"
                            onClick={() => setNotifications({
                              ...notifications,
              ...
              <TabsContent value="activity">
                <Card className="glass-card rounded-2xl">
                  <CardHeader>
                    ...
                    <p className="text-slate-600 dark:text-slate-400">Your account activity and transaction history</p>
                  </CardHeader>
                  <CardContent>
                      ...
                      {activityLog.map((activity, index) => (
                        <div key={index} className="flex items-center space-x-4 p-4 bg-slate-50/50 dark:bg-slate-700/30 rounded-xl">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                            ...
                            activity.type === 'payment' ? 'bg-gradient-to-r from-emerald-500 to-green-600' :
                            activity.type === 'policy' ? 'bg-gradient-to-r from-purple-500 to-indigo-500' :
                            'bg-gradient-to-r from-slate-500 to-slate-600'
                          }`}>
                            {activity.type === 'claim' && <Shield className="w-5 h-5 text-white" />}
                            {activity.type === 'payment' && <CheckCircle className="w-5 h-5 text-white" />}
                            {activity.type === 'policy' && <Lock className="w-5 h-5 text-white" />}
