...
import { reportTemplates, recentReports, quickStats } from '@/public/data/admin/reportsData';
import { 
  Download, 
...
export default function Reports() {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      ...
      case 'claims': return 'from-blue-500 to-cyan-500';
      case 'customers': return 'from-purple-500 to-indigo-500';
      case 'financial': return 'from-green-500 to-emerald-500';
      case 'risk': return 'from-red-500 to-pink-500';
      case 'compliance': return 'from-yellow-500 to-orange-500';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="page-header">
          <div className="page-header-content">
            ...
            <div>
              ...
              <p className="page-header-subtitle">Generate comprehensive reports and analyze business performance</p>
            </div>
          </div>
        </div>
        ...
        <div className="stats-grid">
          <Card className="glass-card rounded-2xl">
            <CardContent className="p-6">
              ...
              <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-1">{quickStats.totalPolicies}</h3>
          ...
          <Card className="glass-card rounded-2xl">
            <CardContent className="p-6">
              ...
              <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-1">{quickStats.totalClaims}</h3>
          ...
          <Card className="glass-card rounded-2xl">
            <CardContent className="p-6">
              ...
              <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-1">{quickStats.totalRevenue}</h3>
          ...
          <Card className="glass-card rounded-2xl">
            <CardContent className="p-6">
              ...
              <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-1">{quickStats.activeUsers}</h3>
        ...
        <Tabs defaultValue="templates" className="space-y-6">
          <TabsList className="grid w-full grid-cols-3 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
            <TabsTrigger value="templates" className="rounded-lg">Report Templates</TabsTrigger>
            <TabsTrigger value="custom" className="rounded-lg">Custom Reports</TabsTrigger>
            <TabsTrigger value="recent" className="rounded-lg">Recent Reports</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="templates">
            <div className="grid lg:grid-cols-2 gap-6">
              {reportTemplates.map((template) => {
                ...
                return (
                  <Card key={template.id} className="glass-card rounded-2xl card-hover">
                    <CardHeader className="pb-4">
                      <div className="flex items-center space-x-3 mb-4">
                        <div className={`w-12 h-12 rounded-xl bg-gradient-to-r ${getCategoryColor(template.category)} flex items-center justify-center`}>
                          <CategoryIcon className="w-6 h-6 text-white" />
                        </div>
                        <div>
                          <CardTitle className="text-lg text-slate-800 dark:text-slate-100">{template.name}</CardTitle>
                          <p className="text-sm text-slate-600 dark:text-slate-400 capitalize">{template.category} Report</p>
                        </div>
                      </div>
                    </CardHeader>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-medium text-slate-800 dark:text-slate-100">{new Date(template.lastGenerated).toLocaleDateString()}</p>
                        </div>
                        <div>
                          <p className="text-sm text-slate-600 dark:text-slate-400">Frequency</p>
                          <p className="font-medium text-slate-800 dark:text-slate-100">{template.frequency}</p>
                        </div>
                      </div>

                      <div>
                        <p className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Available Formats:</p>
          ...
          <TabsContent value="custom">
            <Card className="glass-card rounded-2xl">
              <CardHeader>
                <CardTitle className="text-xl text-slate-800 dark:text-slate-100">Custom Report Builder</CardTitle>
                <p className="text-slate-600 dark:text-slate-400">Create custom reports with specific parameters</p>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="grid md:grid-cols-4 gap-4 p-4 bg-slate-50/50 dark:bg-slate-700/30 rounded-xl">
                  <div className="text-center">
                    ...
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Charts</p>
                    <p className="text-xs text-slate-500 dark:text-slate-500">Include visual charts</p>
                  </div>
                  <div className="text-center">
                    <PieChart className="w-8 h-8 text-slate-500 dark:text-slate-400 mx-auto mb-2" />
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Breakdown</p>
                    <p className="text-xs text-slate-500 dark:text-slate-500">Category breakdown</p>
                  </div>
                  <div className="text-center">
                    <TrendingUp className="w-8 h-8 text-slate-500 dark:text-slate-400 mx-auto mb-2" />
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Trends</p>
                    <p className="text-xs text-slate-500 dark:text-slate-500">Trend analysis</p>
                  </div>
                  <div className="text-center">
                    <Target className="w-8 h-8 text-slate-500 dark:text-slate-400 mx-auto mb-2" />
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-300">KPIs</p>
                    <p className="text-xs text-slate-500 dark:text-slate-500">Key metrics</p>
                  </div>
                </div>
                  ...
                  {paginatedReports.map((report, index) => (
                    <div key={index} className="flex items-center justify-between p-4 bg-slate-50/50 dark:bg-slate-700/30 rounded-xl hover:bg-slate-100/50 dark:hover:bg-slate-700/50 transition-colors">
                      <div className="flex items-center space-x-4">
                        <div className={`w-12 h-12 rounded-xl bg-gradient-to-r ${getCategoryColor(report.category)} flex items-center justify-center`}>
                          <FileText className="w-6 h-6 text-white" />
                        </div>
                        <div>
                          <h3 className="font-semibold text-slate-800 dark:text-slate-100">{report.name}</h3>
                          <p className="text-sm text-slate-600 dark:text-slate-400">{report.type} • {report.size}</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-4">
                        <div className="text-right">
                          <p className="text-sm text-slate-600 dark:text-slate-400">Generated</p>
                          <p className="font-medium text-slate-800 dark:text-slate-100">{new Date(report.generatedDate).toLocaleDateString()}</p>
                        </div>
                        <Badge variant="secondary" className="bg-slate-200 dark:bg-slate-600/50 text-slate-700 dark:text-slate-300">{report.format}</Badge>
